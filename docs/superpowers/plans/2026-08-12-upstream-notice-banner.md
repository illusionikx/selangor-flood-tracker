...
- Modify: `api.php:1291-1299` (the page loop)
- Modify: `api.php:1737` (the payload's `warnings` line)
...
In `api.php`, replace lines 1291 to 1299 exactly:
...
In `api.php`, find line 1737, which reads `    'warnings' => $metWarn,`. Insert this line directly after it:
...
**Files:**
- Modify: `js/config.js` — add `NOTICE` after `HOTLINES` at line 94
...
In `js/config.js`, insert after line 94, the `HOTLINES` export:
...
Then, beside the other `.i-*` class rules, directly after `.i-play_arrow` at line 129, insert:
...
**Files:**
- Modify: `js/alerts.js:158-170` (`warnCard`) and `js/alerts.js:246`
- Modify: `js/ui.js:1114-1122` (the `[data-warn]` handler)
- Modify: `index.html:820-828` (the `warnBox` dialog)
- Modify: `css/chrome.css` — add after `.warnrow .warntext` at line 1242
...
In `js/alerts.js`, replace lines 150 to 170 — the comment block and `warnCard()` — with:

```js
/* Two kinds of regional notice sit above every station in the panel. A MET warning is about the
   weather. An outage notice is about a source that has stated it is down.
   Neither is a station reading, so neither adds anything to the counts below: the badge, the tab
   title, the tally glyphs and the warning glyph still read the station list alone. Merging the two
   tells a reader that stations are in trouble when none is.
   An outage sits above a weather warning. A reader who cannot trust the list has to know that before
   reading the list.
   The row clips its text with CSS, not by cutting the string. The full sentence stays in the DOM for
   a screen reader, and for anyone who copies it. The modal holds all of it. */
const BANNER = {
...
function bannerCard(list, kind) {
  ...
  const b = BANNER[kind];
  // A MET warning carries its own words. An outage carries an id, and the words live in config.js.
  const row = (w, i) => {
    ...
    return `<button class="warnrow" data-banner="${kind}:${i}">
        <b>${t.title}</b>
        <span class="warntext">${t.text}</span>
      </button>`;
  };
...
Add `NOTICE` to the existing `config.js` import at the top of `js/alerts.js`. Find the line beginning
`import {` that already pulls `STATUS_COLOR` and `ALERT_TITLE`, and add `NOTICE` to its list.

- [ ] **Step 2: Draw both, outage first**

In `js/alerts.js`, line 246 currently reads:
...
```js
  // Outage first: a reader who cannot trust the list has to know that before reading the list.
  const warnHtml = bannerCard(state.notices, 'notice') + bannerCard(state.warnings, 'warn');
```
...
In `css/chrome.css`, insert after line 1242, the `.warnrow .warntext` rule:
...
In `index.html`, replace the `warnBox` dialog at lines 820 to 828:
...
In `js/ui.js`, replace lines 1111 to 1122 — the comment and the `[data-warn]` listener:
...
document.addEventListener('click', e => {
  ...
  if (kind === 'notice') {
    ...
    const where = it.regions && it.regions.length
      ? `<p>${andList(it.regions)} water levels may be behind. Stations still show their last known reading.</p>`
      : '';
    el('warnBody').innerHTML = `<h3>${t.title}</h3><p>${t.text}</p>${where}
      <p class="muted">Where JPS says to look instead:</p>
      <ul class="noticelinks">${t.links.map(([label, url]) =>
        `<li><a href="${url}" target="_blank" rel="noopener">${label}</a></li>`).join('')}</ul>`;
  } else {
...
`andList()` is a small helper this file does not have yet. Add it directly above the listener. It is
not called `list`, because `js/ui.js` already binds that name twice, at lines 507 and 702:
...
1. A `Service Notice` card sits above `Forecast Warning` in the alert panel.
...
**Files:**
- Modify: `js/ticker.js:50-59` (the `warns` tiles) and `js/ticker.js:65` and `js/ticker.js:77`
...
In `js/ticker.js`, replace lines 50 to 59 — the comment and the `warns` constant:
...
```js
  ...
  const tile = (kind, icon, c, text, i) =>
    `<button class="tk-i tk-warn" data-banner="${kind}:${i}" tabindex="-1">
      <i class="i i-${icon}" style="--c:${c}"></i>
      <b>${text.title}</b><span class="tk-why">${text.line ?? text.text}</span>
  ...
  const notes = state.notices
    ...
    .map(([t, i]) => tile('notice', 'public_off', 'var(--k-source)', t, i));

  const warns = state.warnings.map((w, i) => tile('warn', 'rainy_heavy', 'var(--k-weather)', w, i));

  const banners = notes.concat(warns);
```

Add `NOTICE` to the `config.js` import at the top of `js/ticker.js`.

- [ ] **Step 2: Keep the quiet state honest**

In `js/ticker.js`, line 65 currently reads:
...
In `js/ticker.js`, line 77 currently begins:
...
In `js/ticker.js`, the comment at lines 125 and 126 names `warns`. Replace those two lines:
...
1. The strip leads with the outage tile, then any weather warning, then the stations.
