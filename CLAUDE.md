...
| `js/map.js` | map instance, basemap/theme, the pin layer, the station panel (`openSide`), `focusOn` / `flashTo`. Also the coverage mask, the zoom floor, the pan limit and the tile skip, all four off `border.json`. The ground stops at a square frame around the circle, and the map asks for no tile outside it. Also `#watertint`, the blue on Dark Matter's water, and `showRivers()`, the river lines on the dark theme. Also the zoom ceiling, 15, and `disableClusteringAtZoom: 15` on the cluster, which is what makes 15 the zoom that merges nothing |
...
| `map-limits-test.html` | `chrome --headless --dump-dom` — one of eight runnable checks. Guards the coverage circle, the zoom floor and the pan limit. It probes the drawn shape with `isPointInFill`, so it reads the fill rule the browser paints with. It asserts that no water pane came back, that the dark water tint paints a lake blue and leaves a road edge in the same grey band grey, and only on a CARTO ground, that the river bands load and follow the zoom on the dark theme alone, that that the circle holds every point of the land ring, that its centre sits east of that ring's middle, that the map asks for no tile outside the square frame around the circle, that the ground colour covers the tiles past that frame, that the faint stripes still carry their hairline, and that the pattern sprite is rendered rather than `display: none`, which is the one thing that empties the tile with nothing to say so |
...
| `water-build.php` | `php water-build.php` — rebakes `water.json` and `rivers.json` from OpenStreetMap. The app reads `rivers.json` alone since 2026-09-14. It runs two Overpass queries. The second asks for `natural=coastline`, to build the sea. `MIN_AREA_KM2` and `MIN_RIVER_KM` are band edges now. They no longer cut a shape. Band 0 rivers run 1.0008 to 94.1182 km. Band 1 runs 0.3003 to 0.9984 km. Band 2 runs 0.0111 to 0.2982 km. Bodies follow the same pattern against 0.01 and 0.002 square kilometres. Also holds the Douglas-Peucker tolerance `TOL_DEG`. Takes `--tol=` and `--dp=` to try a value, and `--cached` to reuse the last raw Overpass answer. Run by hand, never in a request |
...
- `#warnBox`'s icon sits IN LINE with its headline, in a row inside the app b...
...
- THE DARK WATER TINT KEYS ON A GREY BAND AND ON A SHAPE, AND CARTO OWNS THE BAND.
...
- **The layer controls are a bare CHIP ROW over the map, and there is no panel left at all.**
  ...
  **`#paintmenu`, `#paint` and `#navLayers` are all gone.** The chips ARE the control, so a panel, a
  ...
  with no job. The bar carries three items now. `#locate` took the slot beside the zoom box, and on
  2026-09-15 it became the zoom control's first cell, above the plus, at every width. `js/locate.js`
  builds it there. The bar's location item went the same day, and a press opens no card below 600px.
